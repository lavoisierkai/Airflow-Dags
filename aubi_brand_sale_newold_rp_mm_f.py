...
tableName = "aubi_brand_sale_newold_rp_mm_f"
	...
	from( 
	    select order_id 
	        ...
	        ,CASE upgrade_gift_type  
	        	WHEN 1 THEN "10 VIP直升"
	        	WHEN 2 THEN "30 SVIP直升"
	        	WHEN 3 THEN "31 SVIP自选"
	        	WHEN 4 THEN "20 VVIP直升"
	        	WHEN 5 THEN "21 VVIP自选"
	        	ELSE  		"00 非礼包"
...
smr as (
    select stat_date,rp_month
        ...
        ,sum(is_first_brand_order) as first_byr_cnt
        ,sum(is_first_order) as vtn_first_byr_cnt
        ,sum(is_first_brand_order_strict) as vtn_new_mostly_led_by_brand
        ...
        ,sum(re_purchase*is_first_brand_order) as brd_new_rp_buyer_cnt  
    from user_smr 
...
select *  
    ...
    ,vtn_rp_buyer_cnt/buyer_cnt as vtn_rp_rate
from smr
order by brand_id,stat_date
