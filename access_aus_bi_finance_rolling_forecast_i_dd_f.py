...
tableName = "access_aus_bi_finance_rolling_forecast_i_dd_f"
...
dag = DAG(
    ...
    description="access_aus_bi.access_aus_bi_finance_rolling_forecast_i_dd_f_AGG",
...
querry = '''
SELECT cast(brand_id as BIGINT) brand_id,
        substr(predict_time,1,7) as predict_month,
        substr(dt,1,7) as dt,
        sum(gmv) AS predicted_GMV,
        sum(quantity) AS predicted_quantity
FROM access_aus_bi.access_aus_bi_finance_rolling_forecast_i_dd_f
WHERE dt = months_add(trunc(predict_time,'MM'),-1) 
GROUP BY brand_id, substr(predict_time,1,7),substr(dt,1,7)
order by substr(predict_time,1,7) desc
union all
 SELECT 0 as brand_id,
        substr(predict_time,1,7) as predict_month,
        substr(dt,1,7) as dt,
        sum(gmv) AS predicted_GMV,
        sum(quantity) AS predicted_quantity
